...
(function (Bonziri) {
    ...
    var _Sprite = (function () {
        function _Sprite(label, obj, rc, ef) {
            this.label = label;
    ...
    var _Sound = (function () {
        function _Sound(label, obj, rc, ef) {
            this.label = label;
    ...
    var _Cast = (function () {
        function _Cast(label, obj, stuff) {
            this.label = label;
    ...
    var _LinkText = (function () {
        function _LinkText(idx, text, line_label) {
            this.text = text;
    ...
    var _Caption = (function () {
        function _Caption(idx, name, speed, text) {
            this.name = name;
            this.speed = speed;
            this.text = text;
            if (speed <= 0) {
    ...
    var _Line = (function () {
        function _Line(idx, obj, scene) {
            ...
            if (Array.isArray(obj)) {
                ...
                if (lastLine && lastLine.caption) {
                    this.caption = new _Caption(idx, lastLine.caption.name, lastLine.caption.speed, null);
                }
                else {
                ...
                if (lastLine && lastLine.cast && lastLine.caption) {
                    ...
                    this.caption = new _Caption(idx, lastLine.caption.name, lastLine.caption.speed, obj);
                }
                else {
                ...
                if ('action' in obj) {
                    if (typeof obj['action'] === 'string') {
                        ...
                        this.action = new _Action('', obj['action'], ef);
                    }
                }
                else {
                }
                ...
                if ('caption' in obj) {
                    if (typeof obj['caption'] === 'string' ||
                        ...
                        if ('cast' in obj) {
                            this.caption = new _Caption(idx, this.cast.name, 5, null);
                        }
                        ...
                        var c = obj['caption'];
                        if ('cast' in obj) {
                            this.caption = new _Caption(idx, c.name ? c.name : this.cast.name, c.speed ? c.speed : 5, null);
                        }
                        else if (lastLine && lastLine.cast && lastLine.caption) {
                            this.cast = lastLine.cast;
                            this.caption = new _Caption(idx, c.name ? c.name : lastLine.caption.name, c.speed ? c.speed : lastLine.caption.speed, null);
                        }
                        if (c.text) {
        ...
        _Line.prototype._setCast = function (idx, casts, label) {
            if (label in casts) {
        ...
        _Line.prototype._setAction = function (idx, actions, label) {
            if (label in actions) {
        ...
        _Line.prototype._setTarget = function (idx, key) {
            if (key in LineTarget) {
                var val = LineTarget[key];
                this.target = val;
            }
            else {
        ...
        _Line.prototype._setSprites = function (idx, cast, arg) {
        ...
        _Line.prototype._setSounds = function (idx, cast, arg) {
            if (Array.isArray(arg)) {
                var ids = arg;
                for (var i = 0; i < ids.length; i += 1) {
                    if (typeof ids[i] === 'string') {
        ...
        _Line.prototype._setCaptionText = function (idx, arg) {
            if (Array.isArray(arg)) {
                var src = arg;
                if (src.length == 1) {
                    if (typeof src[0] == 'string') {
                        ...
                        var lt = new _LinkText(idx, src[0][0], src[0][1]);
                    ...
                    for (var i = 0; i < src.length; i++) {
                        if (typeof src[i] === 'string') {
                            ...
                            var lt = new _LinkText(idx, src[i][0], src[i][1]);
                            dst.push(lt);
                        }
                        else {
    ...
    var _Scenario = (function () {
        function _Scenario(jsonText) {
            ...
            if ('lines' in this._obj && Array.isArray(this._obj['lines'])) {
                ...
                for (var idx = 0; idx < ls.length; idx += 1) {
                    var l = new _Line(idx, ls[idx], this);
                    this.lines.push(l);
        ...
        _Scenario.prototype.getLineIndex = function (label) {
    ...
    var Serif = (function () {
        ...
        Serif.prototype.getText = function () {
            return this._message.slice(0, this._position);
        };
        Serif.prototype.isSpeaking = function () {
            return this._position < this._message.length;
        };
        Serif.prototype.reset = function () {
            this._position = 0;
        };
        return Serif;
    })();
...
function JsonRequestListener() {
    ...
    var d = new Director('gameContent', _B.generateScenario(txt));
    var el = $('#inputText');
    el.val(txt);
